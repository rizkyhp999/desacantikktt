...
export default function DataViewerClient() {
  // State Password Protection ("buongbaru")
  ...
  useEffect(() => {
    const authStatus = sessionStorage.getItem("descan_data_authenticated");
    if (authStatus === "true") {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput.trim() === "buongbaru") {
      setIsAuthenticated(true);
      setPasswordError("");
      sessionStorage.setItem("descan_data_authenticated", "true");
    } else {
  ...
  const handleLogout = () => {
    sessionStorage.removeItem("descan_data_authenticated");
  ...
  if (isCheckingAuth) {
    return (
      <div className="w-full max-w-7xl mx-auto px-4 py-20 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-[#6c6a64]">Memeriksa otorisasi akses data...</p>
      </div>
    );
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex items-center justify-center">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }}
          className="w-full max-w-md bg-white border border-[#e6dfd8] rounded-2xl p-6 sm:p-8 shadow-xl space-y-6"
        >
          <div className="text-center space-y-3">
            ...
            <div className="space-y-1">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 text-xs font-semibold uppercase tracking-wider border border-emerald-200">
                <ShieldCheck className="w-3.5 h-3.5" /> Akses Terproteksi
              </span>
              <h2 className="text-xl sm:text-2xl font-bold text-[#141413] pt-1">
                Password Akses Data
              </h2>
              <p className="text-xs sm:text-sm text-[#6c6a64]">
                Silakan masukkan password untuk melihat dan mengelola Data Desa Buong Baru.
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label htmlFor="data-password-input" className="block text-xs font-bold text-[#141413]">
                Password
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[#6c6a64]">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full pl-9 pr-10 py-2.5 bg-[#faf9f5] border border-[#e6dfd8] rounded-xl text-sm text-[#141413] focus:outline-hidden focus:ring-2 focus:ring-emerald-600 focus:bg-white transition-all placeholder:text-[#99968d]"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-[#6c6a64] hover:text-[#141413] transition-colors cursor-pointer"
            ...
            <button
              type="submit"
              className="w-full py-2.5 px-4 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-sm rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2"
            >
              <Lock className="w-4 h-4" />
              <span>Buka Akses Data</span>
            </button>
          </form>

          <div className="text-center pt-2 border-t border-[#e6dfd8]">
            <p className="text-[11px] text-[#99968d]">
              Sistem Data Terpadu Desa Cantik (Descan) Buong Baru 2026
            </p>
          </div>
        </motion.div>
      </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-full hover:bg-red-100 transition-colors cursor-pointer"
              title="Kunci Kembali Akses Data"
            >
